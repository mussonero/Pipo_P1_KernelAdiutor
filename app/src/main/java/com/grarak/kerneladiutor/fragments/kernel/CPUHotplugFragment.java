...
public class CPUHotplugFragment extends RecyclerViewFragment implements
    ...
    private SeekBarCardView.DSeekBarCard mRkSmartCoresNrBoostCups;
    private SeekBarCardView.DSeekBarCard mRkSmartCoresSampleRate;
    private SeekBarCardView.DSeekBarCard mRkSmartCoresIdleThreshold;
    private SeekBarCardView.DSeekBarCard mRkSmartCoresBusyThreshold;
    private SeekBarCardView.DSeekBarCard mRkSmartCoresBoostDownDelay;
    ...
    public void init(Bundle savedInstanceState) {
        ...
        if (CPUHotplug.hasRkSmartCores()) rkSmartCoresInit();
    }
    ...
    private void rkSmartCoresInit() {
        List<DAdapter.DView> views = new ArrayList<>();

        if (CPUHotplug.hasRkSmartCoresNrBoostCups()) {
            List<String> list = new ArrayList<>();
            for (int i = 0; i <= CPU.getCoreCount(); i++) list.add(String.valueOf(i));

            mRkSmartCoresNrBoostCups = new SeekBarCardView.DSeekBarCard(list);
            mRkSmartCoresNrBoostCups.setTitle(getString(R.string.nr_boost_cpus));
            mRkSmartCoresNrBoostCups.setDescription(getString(R.string.nr_boost_cpus_summary));
            mRkSmartCoresNrBoostCups.setProgress(CPUHotplug.getRkSmartCoresNrBoostCups());
            mRkSmartCoresNrBoostCups.setOnDSeekBarCardListener(this);

            views.add(mRkSmartCoresNrBoostCups);
        }

        if (CPUHotplug.hasRkSmartCoresSampleRate()) {
            List<String> list = new ArrayList<>();
            list.add(getString(R.string.disabled));
            for (int i = 1; i < 101; i++) list.add(i + "%");

            mRkSmartCoresSampleRate = new SeekBarCardView.DSeekBarCard(list);
            mRkSmartCoresSampleRate.setTitle(getString(R.string.sample_rate));
            mRkSmartCoresSampleRate.setDescription(getString(R.string.sample_rate_summary));
            mRkSmartCoresSampleRate.setProgress(CPUHotplug.getRkSmartCoresSampleRate());
            mRkSmartCoresSampleRate.setOnDSeekBarCardListener(this);

            views.add(mRkSmartCoresSampleRate);
        }

        if (CPUHotplug.hasRkSmartCoresIdleThreshold()) {
            List<String> list = new ArrayList<>();
            list.add(getString(R.string.disabled));
            for (int i = 1; i < 101; i++) list.add(i + "%");

            mRkSmartCoresIdleThreshold = new SeekBarCardView.DSeekBarCard(list);
            mRkSmartCoresIdleThreshold.setTitle(getString(R.string.idle_threshold));
            mRkSmartCoresIdleThreshold.setDescription(getString(R.string.idle_threshold_summary));
            mRkSmartCoresIdleThreshold.setProgress(CPUHotplug.getRkSmartCoresIdleThreshold());
            mRkSmartCoresIdleThreshold.setOnDSeekBarCardListener(this);

            views.add(mRkSmartCoresIdleThreshold);
        }

        if (CPUHotplug.hasRkSmartCoresBusyThreshold()) {
            List<String> list = new ArrayList<>();
            list.add(getString(R.string.disabled));
            for (int i = 1; i < 101; i++) list.add(i + "%");

            mRkSmartCoresBusyThreshold = new SeekBarCardView.DSeekBarCard(list);
            mRkSmartCoresBusyThreshold.setTitle(getString(R.string.busy_threshold));
            mRkSmartCoresBusyThreshold.setDescription(getString(R.string.busy_threshold_summary));
            mRkSmartCoresBusyThreshold.setProgress(CPUHotplug.getRkSmartCoresBusyThreshold());
            mRkSmartCoresBusyThreshold.setOnDSeekBarCardListener(this);

            views.add(mRkSmartCoresBusyThreshold);
        }

        if (CPUHotplug.hasRkSmartCoresBoostDownDelay()) {
            List<String> list = new ArrayList<>();
            for (int i = 0; i < 501; i++)
                list.add((i) + getString(R.string.ms));

            mRkSmartCoresBoostDownDelay = new SeekBarCardView.DSeekBarCard(list);
            mRkSmartCoresBoostDownDelay.setTitle(getString(R.string.boost_down_delay));
            mRkSmartCoresBoostDownDelay.setDescription(getString(R.string.boost_down_delay_summary));
            mRkSmartCoresBoostDownDelay.setProgress(CPUHotplug.getRkSmartCoresBoostDownDelay());
            mRkSmartCoresBoostDownDelay.setOnDSeekBarCardListener(this);

            views.add(mRkSmartCoresBoostDownDelay);
        }

        if (views.size() > 0) {
            DDivider mRkSmartCoresDividerCard = new DDivider();
            mRkSmartCoresDividerCard.setText(getString(R.string.rk_smartcores));
            mRkSmartCoresDividerCard.setDescription(getString(R.string.rk_smartcores_summary));
            addView(mRkSmartCoresDividerCard);
    ...
    public void onStop(SeekBarCardView.DSeekBarCard dSeekBarCard, int position) {
        ...
        else if (dSeekBarCard == mRkSmartCoresNrBoostCups)
            CPUHotplug.setRkSmartCoresNrBoostCups(position, getActivity());
        else if (dSeekBarCard == mRkSmartCoresSampleRate)
            CPUHotplug.setRkSmartCoresSampleRate(position, getActivity());
        else if (dSeekBarCard == mRkSmartCoresIdleThreshold)
            CPUHotplug.setRkSmartCoresIdleThreshold(position, getActivity());
        else if (dSeekBarCard == mRkSmartCoresBusyThreshold)
            CPUHotplug.setRkSmartCoresBusyThreshold(position, getActivity());
        else if (dSeekBarCard == mRkSmartCoresBoostDownDelay)
            CPUHotplug.setRkSmartCoresBoostDownDelay(position, getActivity());
    }
