...
public class GPUFragment extends RecyclerViewFragment implements PopupCardView.DPopupCard.OnDPopupCardListener,
    ...
    private CardViewItem.DCardView mCur2dFreqCard, mCurFreqCard, mAutilisationCard, mGpuTempCard;
    ...
    private PopupCardView.DPopupCard  mGpuCoresCard;
    ...
    private void curFreqInit() {
        ...
        if (GPU.hasGpuAutilisation()) {
            mAutilisationCard = new CardViewItem.DCardView();
            mAutilisationCard.setTitle(getString(R.string.gpu_autilisation));

            addView(mAutilisationCard);
        }

        if (GPU.hasGpuTemp()) {
            mGpuTempCard = new CardViewItem.DCardView();
            mGpuTempCard.setTitle(getString(R.string.gpu_temp));

            addView(mGpuTempCard);
        }
    ...
    private void governorInit() {
        ...
        if (GPU.hasGpuCore()) {
            mGpuCoresCard = new PopupCardView.DPopupCard(GPU.getGpuCores());
            mGpuCoresCard.setTitle(getString(R.string.gpu_core));
            mGpuCoresCard.setDescription(getString(R.string.gpu_core_summary));
            mGpuCoresCard.setItem(GPU.getGpuCore());
            mGpuCoresCard.setOnDPopupCardListener(this);

            addView(mGpuCoresCard);
        }
    }
    ...
    public void onItemSelected(PopupCardView.DPopupCard dPopupCard, int position) {
        ...
        else if (dPopupCard == mGpuCoresCard)
            GPU.setGpuCore(GPU.getGpuCores().get(position), getActivity());
    }
    ...
    public boolean onRefresh() {
        ...
        if (mAutilisationCard != null)
            mAutilisationCard.setDescription((GPU.getGpuAutilisation() / 1) + "%");

        if (mGpuTempCard != null)
            mGpuTempCard.setDescription((GPU.getGpuTemp() / 1) + "C°");
