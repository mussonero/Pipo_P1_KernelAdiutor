...
public class GPU implements Constants {
    ...
    private static String GPU_AUTILISATION;
    private static String GPU_TEMP;
    ...
    private static String GPU_SCALING_CORE;
    private static String[] GPU_AVAILABLE_CORES;
    ...
    public static void setGpuCore(String core, Context context) {
        if (GPU_SCALING_CORE != null)
            Control.runCommand(core, GPU_SCALING_CORE, Control.CommandType.GENERIC, context);
    }

    public static List<String> getGpuCores() {
        if (GPU_AVAILABLE_CORES == null)
            for (String file : GPU_AVAILABLE_CORES_ARRAY)
                if (GPU_AVAILABLE_CORES == null)
                    if (Utils.existFile(file)) {
                        String value = Utils.readFile(file);
                        if (value != null)
                            GPU_AVAILABLE_CORES = value.split(" ");
                    }
        return new ArrayList<>(Arrays.asList(GPU_AVAILABLE_CORES == null ? GPU_GENERIC_CORES
                .split(" ") : GPU_AVAILABLE_CORES));
    }

    public static String getGpuCore() {
        if (GPU_SCALING_CORE != null)
            if (Utils.existFile(GPU_SCALING_CORE)) {
                String value = Utils.readFile(GPU_SCALING_CORE);
                if (value != null) return value;
            }
        return "";
    }

    public static boolean hasGpuCore() {
        if (GPU_SCALING_CORE == null)
            for (String file : GPU_SCALING_CORE_ARRAY)
                if (Utils.existFile(file)) GPU_SCALING_CORE = file;
        return GPU_SCALING_CORE != null;
    }
    ...
    public static int getGpuAutilisation() {
        if (GPU_AUTILISATION != null) if (Utils.existFile(GPU_AUTILISATION)) {
            String value = Utils.readFile(GPU_AUTILISATION);
            if (value != null) return Utils.stringToInt(value);
        }
        return 0;
    }

    public static boolean hasGpuAutilisation() {
        if (GPU_AUTILISATION == null) {
            for (String file : GPU_AUTILISATION_ARRAY)
                if (Utils.existFile(file)) GPU_AUTILISATION = file;
        }
        return GPU_AUTILISATION != null;
    }

    public static int getGpuTemp() {
        if (GPU_TEMP != null) if (Utils.existFile(GPU_TEMP)) {
            String value = Utils.readFile(GPU_TEMP);
            if (value != null) return Utils.stringToInt(value);
        }
        return 0;
    }

    public static boolean hasGpuTemp() {
        if (GPU_TEMP == null) {
            for (String file : GPU_TEMP_ARRAY)
                if (Utils.existFile(file)) GPU_TEMP = file;
        }
        return GPU_TEMP != null;
    }
